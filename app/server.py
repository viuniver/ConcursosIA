...
Rotas:
  ...
  GET /api/busca-semantica     → expande termo de cargo em termos relacionados
...
def _aplicar_filtros(
    vagas: list[dict],
    params: dict,
    termos_cargo: list[str] | None = None,
    termos_orgao: list[str] | None = None,
) -> list[dict]:
    hoje = _hoje_iso()
    resultado = []
    ...
    for v in vagas:
        ...
        # Cargo — semântico (lista de termos) OU substring simples
        if termos_cargo:
            cargo_vaga = str(v.get("cargo") or "").lower()
            if not any(t in cargo_vaga for t in termos_cargo):
                continue
        elif cargo_q:
            if cargo_q not in str(v.get("cargo") or "").lower():
                continue

        # Órgão — semântico (lista de termos) OU substring simples
        if termos_orgao:
            orgao_vaga = str(v.get("orgao") or "").lower()
            if not any(t in orgao_vaga for t in termos_orgao):
                continue
        elif orgao_q:
            if orgao_q not in str(v.get("orgao") or "").lower():
                continue
...
@app.route("/api/busca-semantica")
def api_busca_semantica():
    """Expande termo de cargo em cargos relacionados."""
    query = request.args.get("q", "").strip()
    if not query:
        return jsonify({"query": "", "termos": []})
    termos = expandir_query_cargo(query)
    return jsonify({"query": query, "termos": termos})


@app.route("/api/busca-semantica-orgao")
def api_busca_semantica_orgao():
    """Expande termo de órgão em nomes relacionados."""
    query = request.args.get("q", "").strip()
    if not query:
        return jsonify({"query": "", "termos": []})
    termos = expandir_query_orgao(query)
    return jsonify({"query": query, "termos": termos})
...
def api_vagas():
    ...
    # Expansão semântica do cargo (se houver query de cargo)
    cargo_q = request.args.get("cargo", "").strip()
    orgao_q = request.args.get("orgao", "").strip()
    termos_cargo: list[str] | None = None
    termos_orgao: list[str] | None = None
    busca_semantica: dict | None = None
    busca_semantica_orgao: dict | None = None

    if cargo_q:
        termos_cargo = expandir_query_cargo(cargo_q)
        busca_semantica = {"query": cargo_q, "termos": termos_cargo}

    if orgao_q:
        termos_orgao = expandir_query_orgao(orgao_q)
        busca_semantica_orgao = {"query": orgao_q, "termos": termos_orgao}

    filtradas = _aplicar_filtros(vagas, request.args, termos_cargo, termos_orgao)
    ...
    return jsonify({
        ...
        "busca_semantica_orgao": busca_semantica_orgao,
    })
